...
type SchoolArticleCardProps = {
  project: Project;
  delay: number;
};

export function SchoolArticleCard({ project, delay }: SchoolArticleCardProps) {
  // 기술 스택 처리
  ...
  return (
    <BlurFade delay={delay}>
      <Link
        ...
        href={`/projects/schools/${project.id}`}
      >
        <div className="flex flex-1 flex-col space-y-4 p-2">
          {/* 헤더 영역 */}
          <div className="flex flex-col space-y-1">
            {project.metadata.school && (
              <Badge variant="outline" className="inline-flex w-fit items-center gap-1.5">
