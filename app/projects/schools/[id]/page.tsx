...
export async function generateMetadata({
  ...
  const project = await getProject(id, 'schools');
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: 'article',
      url: `${DATA.url}/projects/schools/${project.id}`,
      images: [
        {
          url: ogImage,
        },
...
export default async function ProjectDetailPage({
  ...
  const breadcrumbItems = [
    ...
    { label: 'Academic Projects', href: '/projects/schools' },
    ...
    <section id="project" className="mx-auto max-w-4xl px-6 pt-20 pb-40">
      <BlurFade delay={BLUR_FADE_DELAY} duration={0.5} blur="4px">
        ...
        {project.metadata.school && (
          <Badge variant="outline" className="mb-2 inline-flex w-fit items-center gap-1.5">
            <GraduationCap className="size-3.5" />
            <span>{project.metadata.school}</span>
          </Badge>
        )}
